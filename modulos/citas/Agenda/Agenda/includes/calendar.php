...
function userMenu()
{
	...
	if($action != 'settings')
		menu_item_append($span, __('Configuración'), 'settings');
...
function navbar()
{
	...
	if($action != 'search') {
		menu_item_append($html, __('Buscar'), 'search', $args);
	}

	if($action != 'display_month') {
		menu_item_append($html, __('Ver Mes'), 'display_month',
			$args);
	}
	...
	if($phpc_cal->can_admin() && $action != 'cadmin') {
		menu_item_append($html, __('Administrar Calendario'), 'cadmin');
	}

	if(is_admin() && $action != 'admin') {
		menu_item_append($html, __('Administrador'), 'admin');
	}

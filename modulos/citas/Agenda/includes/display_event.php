...
function display_event_by_oid($oid)
{
	...
	// Add modify/delete links if this user has access to this event.
		...
		$menu_tag->add(array(create_event_link(__('Modify'),
					...
					create_event_link(__('Delete'),
						'event_delete', $eid), "\n",
					create_occurrence_link(__('Modificar Fecha'),
						'occur_form', $oid), "\n",
					create_occurrence_link(__('Borrar Fecha'),
	...
	$menu_tag->add(create_event_link(__('Ver todas las Fechas'),
				'display_event', $eid));
...
function display_event_by_eid($eid)
{
	...
	$event_header->add(tag('div', __('Creado en: '), $event->ctime));
	if(!empty($event->mtime))
		$event_header->add(tag('div', __('Last modified at: '),
				$event->mtime));
	...
	// Add modify/delete links if this user has access to this event.
		...
		$event_header->add(tag('div', attrs('class="phpc-bar ui-widget-content"'),
					create_event_link(__('Modify'),
						'event_form', $eid), "\n",
					create_event_link(__('Adicionar Fecha'),
						'occur_form', $eid), "\n",
					create_event_link(__('Delete'),
						'event_delete', $eid)));
	}
	...
	foreach($occurrences as $occurrence) {
		...
		if($event->can_modify()) {
			$occ_tag->add(" ",
					create_occurrence_link(__('Editar'), 'occur_form', $oid), " ",
					create_occurrence_link(__('Eliminar'), 'occurrence_delete', $oid));
		}
	...
	return tag('div', attributes('class="phpc-main phpc-event"'),
			tag('h2', $event->get_subject()), $event_header,
			$desc_tag, tag ('div',attributes('class="phpc-occ"'),tag('h3', __('Fecha')),
			$occurrences_tag));
}

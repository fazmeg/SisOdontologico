...
function display_form() {
	...
	$form->add_part(new FormFreeQuestion('subject', __('Para:'),
				false, $phpc_cal->subject_max, true));
	$form->add_part(new FormLongFreeQuestion('description',
				__('Description')));

	$when_group = new FormGroup(__('Fecha'), 'phpc-when');
	if(isset($vars['eid'])) {
	...
	$when_group->add_part(new FormDateTimeQuestion('start',
				__('Comienzo'), $hour24, $date_format));
	$when_group->add_part(new FormDateTimeQuestion('end', __('Fin'),
				$hour24, $date_format));

	$time_type = new FormDropDownQuestion('time-type', __('Tipo de Cita'));
	$time_type->add_option('normal', __('Normal'));
	$time_type->add_option('full', __('Full Day'));
	$time_type->add_option('tba', __('Por Confirmar'));
	...
	$repeat_type->add_option('daily', __('Diario'), NULL, $daily_group);
	...
	$repeat_type->add_option('yearly', __('Anual'), NULL, $yearly_group);
